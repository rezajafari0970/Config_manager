...
def run(port,url,upload=False,timeout_ms=None):
 cfg=load();limit=max(1,min(cfg['probe_timeout'],(timeout_ms or cfg['probe_timeout']*1000)/1000));cmd=['/usr/bin/curl','-fsS','--connect-timeout',str(cfg['connect_timeout']),'--max-time',str(limit),'--socks5-hostname',f'127.0.0.1:{port}'];payload=None
