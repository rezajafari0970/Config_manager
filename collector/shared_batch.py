...
def finish(row,pr,attempt=1):
 u=bool(pr.get('upload',{}).get('ok'));d=bool(pr.get('download',{}).get('ok'));action=initial_action(attempt,u,d);state={'promote_healthy':'healthy','retry_after_30s':'retry','delete':'remove'}[action];now=time.time();c=connect();c.execute('INSERT INTO health_attempts(fingerprint,attempt,phase,upload_ok,download_ok,external_ok,server_traffic_ok,providers,bytes_up,bytes_down,started_at,finished_at,result) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',(row['fingerprint'],attempt,'shared',int(u),int(d),int(u and d),int(u and d),json.dumps({'probe':pr,'shared':True}),pr.get('upload',{}).get('bytes',0),pr.get('download',{}).get('bytes',0),now,now,state));c.execute('UPDATE test_candidates SET upload_ok=?,download_ok=?,healthy=?,stage=?,updated_at=? WHERE fingerprint=?',(int(u),int(d),int(state=='healthy'),'tested' if state!='retry' else 'retry_wait',now,row['fingerprint']));c.execute('DELETE FROM health_claims WHERE fingerprint=?',(row['fingerprint'],));c.commit();c.close();return state
def run(n=8):
 rows=claim_batch(n)
