...
from .vmess_adapter import xray,singbox
from .singbox_core import check
STATE='/root/Config_manager/data/vmess_core_state.json'
def load():
...
def step():
 s=load();c=connect();rows=c.execute("select id,raw from configs where kind='vmess' order by id").fetchall();c.close();s['total']=len(rows)
