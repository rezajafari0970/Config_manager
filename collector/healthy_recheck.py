...
def one():
 ...
 c=connect()
 if pr.get('defer'):
  c.execute('UPDATE healthy_configs SET next_check=? WHERE fingerprint=?',(time.time()+30,r['fingerprint']))
 elif ok:c.execute('UPDATE healthy_configs SET upload_ok=1,download_ok=1,last_healthy=?,next_check=? WHERE fingerprint=?',(time.time(),time.time()+load()['recheck_seconds'],r['fingerprint']))
