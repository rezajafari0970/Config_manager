...
def one():
 ...
 c=connect()
 if ok:c.execute('UPDATE healthy_configs SET upload_ok=1,download_ok=1,last_healthy=?,next_check=? WHERE fingerprint=?',(time.time(),time.time()+load()['recheck_seconds'],r['fingerprint']))
