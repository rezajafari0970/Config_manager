...
def step():
 ...
 if q<100:direction=0
 elif psi>60:direction=-1
 elif best and tpm<best*.88:direction=-1
 elif tpm>=best*.97 and psi<45:direction=1
 conc=max(4,min(16,conc+direction)) if q>=100 else conc;batch=max(4,min(16,s.get('batch_size',8)+(2*direction)));lanes=1 if psi>45 else max(1,min(3,s.get('lanes',1)+(1 if direction>0 and tpm<200 else 0)))
