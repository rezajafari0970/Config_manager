...
def test_one(row,attempt=1):
 ...
 if h.get('ok'):
  try:
   before=snapshot(h);d,dd=download(h['port']);u,uu=upload(h['port']);after=snapshot(h);traffic=verified(before,after);details.update(download=dd,upload=uu,traffic_verified=traffic,io_delta=after['io']-before['io']);d=d and traffic;u=u and traffic
  finally:stop(h)
