...
def step():
 c=connect();now=time.time();tele=server_snapshot();rows=c.execute('SELECT upload_ok,download_ok,started_at,finished_at FROM health_attempts WHERE finished_at>=?',(now-180,)).fetchall();c.close();dur=[(r['finished_at']-r['started_at'])*1000 for r in rows];cfg=load();sys=snapshot();providers=len({x.get('base') for x in active()});
 if providers<cfg['min_active_providers']:
  state={'target_tpm':200,'actual_tpm':round(len(rows)/3,1),'providers':providers,'paused_for_provider_quorum':True,'settings':cfg,'telemetry':tele,'updated':now};P.write_text(json.dumps(state));return state
